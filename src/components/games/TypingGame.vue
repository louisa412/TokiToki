...
onMounted(() => {
  ...
  timer = setInterval(() => {
    ...
    if (timeLeft.value <= 0) {
      ...
      resultMsg.value = gameLine(store, '時間到了。慢。', '時間到了。{target}：差一點。')
      store.endGame('sad', gameMsgs(store, ['打這麼慢？', '...廢。', '下次快一點。'], ['{target}：沒關係。', '下次一起再練。']), 5, -8, 2)
    }
  }, 1000)
})
...
function checkTyped() {
  ...
  if (typed.value.trim() === currentWord.value) {
    ...
    resultMsg.value = bonus
      ? gameLine(store, '速度不錯。這次承認你。', '{target}：速度很快。')
      : gameLine(store, '...勉強及格。', '{target}：剛好趕上了。')
    const dm = bonus ? 15 : 8
    const da = bonus ? 10 : 5
    store.endGame(
      'praised',
      gameMsgs(store, [bonus ? '速度可以。' : '...還行。', '比我想的快一點。', '勉強過了。'], [bonus ? '{target}：打得很快。' : '{target}：有趕上。', '看你這樣很安心。']),
      dm, -5, da
    )
  }
}
