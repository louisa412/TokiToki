<template>
  <div>
    <div class="gp">出拳！</div>
    <div class="gb" v-if="!result">
      <button class="gcb" @click="play('rock')">   <span class="bg">✊</span>石頭</button>
      <button class="gcb" @click="play('scissors')"><span class="bg">✌️</span>剪刀</button>
      <button class="gcb" @click="play('paper')">  <span class="bg">✋</span>布</button>
    </div>
    <div v-if="result" class="gr" :class="result.cls" v-html="result.html"></div>
  </div>
</template>

<script setup>
import { ref } from 'vue'
import { usePetStore } from '../../stores/pet'
import { gameLine, gameMsgs, gameTargetName } from './gameTarget'

const store = usePetStore()
const emit  = defineEmits(['done'])
const result = ref(null)

const OPTS = ['rock', 'scissors', 'paper']
const NM   = { rock: '✊石頭', scissors: '✌️剪刀', paper: '✋布' }

function play(choice) {
  const opponent = OPTS[Math.floor(Math.random() * 3)]
  let cls, txt, dm, da

  if (choice === opponent) {
    cls = 'rd'; txt = gameLine(store, '平手。又來？', '平手。再試一次嗎？'); dm = 3; da = 2
  } else if (
    (choice === 'rock'     && opponent === 'scissors') ||
    (choice === 'scissors' && opponent === 'paper') ||
    (choice === 'paper'    && opponent === 'rock')
  ) {
    cls = 'rw'; txt = gameLine(store, '你贏了。手氣好而已。', `你贏了。${store.activeVisitorName}：反應很快。`); dm = 8; da = 5
  } else {
    cls = 'rl'; txt = gameLine(store, '輸了？哈。果然。', `${store.activeVisitorName} 贏了。${store.activeVisitorName}：剛好猜對了。`); dm = 12; da = 5
  }

  result.value = { cls, html: `${gameTargetName(store)}出了 ${NM[opponent]}<br><span class="${cls}">${txt}</span>` }
  store.endGame(
    'happy',
    gameMsgs(
      store,
      cls === 'rl' ? ['哈。輸了吧。', '就這樣。'] : ['...只是手滑。', '下次不會輸的。'],
      cls === 'rl' ? ['{visitor}：我贏了。', '下次再一起玩吧。'] : ['{visitor}：你很會猜。', '這局很好玩。']
    ),
    dm, -5, da
  )
}
</script>
